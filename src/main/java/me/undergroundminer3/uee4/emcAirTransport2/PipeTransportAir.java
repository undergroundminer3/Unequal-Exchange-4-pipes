...
public class PipeTransportAir extends PipeTransport implements IModdedPipeRenderable, IEmcPacketAble {
	...
	public boolean canPipeConnect(TileEntity tile, ForgeDirection side) {
		if (tile instanceof TileGenericPipe) {
			...
			if (BlockGenericPipe.isValid(pipe2) && !(pipe2.transport instanceof PipeTransportAir)) {
				return false;
			}
			return true;
		}
		...
		if (tile instanceof IAirReceptor) {
			IAirReceptor receptor = (IAirReceptor) tile;
			AirHandler.AirReceiver receiver = receptor.getAirReceiver(side.getOpposite());
			if (receiver != null && receiver.getType().canReceiveFromPipes()) {
				return true;
			}
		}

		if (container.pipe instanceof PipeAirWood && tile instanceof IAirEmitter) {
			IAirEmitter emitter = (IAirEmitter) tile;
			if (emitter.canEmitAirFrom(side.getOpposite())) {
				return true;
			}
		}
	...
	public void updateEntity() {
			...
			if (internalPower[i] > 0) {
					...
					if (j != i && powerQuery[j] > 0) {
						if (tiles[j] != null
								&& (tiles[j] instanceof TileGenericPipe
										|| tiles[j] instanceof IAirReceptor || EnergyAPI
				...
				for (int j = 0; j < 6; ++j) {
					if (j != i && powerQuery[j] > 0) {
						...
						AirHandler.AirReceiver prov = getReceiverOnSide(ForgeDirection.VALID_DIRECTIONS[j]);
						if (prov != null && prov.powerRequest() > 0) {
							watts = (internalPower[i] / totalPowerQuery) * powerQuery[j];
							watts = prov.receiveEnergy(AirHandler.Type.PIPE, watts, ForgeDirection.VALID_DIRECTIONS[j].getOpposite());
							internalPower[i] -= watts;
						} else if (tiles[j] instanceof TileGenericPipe) {
							watts = (internalPower[i] / totalPowerQuery) * powerQuery[j];
							TileGenericPipe nearbyTile = (TileGenericPipe) tiles[j];

							PipeTransportAir nearbyTransport = (PipeTransportAir) nearbyTile.pipe.transport;
		...
		for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
			...
			AirHandler.AirReceiver prov = getReceiverOnSide(dir);
			if (prov != null) {
	...
	private AirHandler.AirReceiver getReceiverOnSide(ForgeDirection side) {
...
//			}
	...
	public double receiveEnergy(ForgeDirection from, double valI) {
		double val = valI;
		step();
		if (this.container.pipe instanceof IPipeTransportAirHook) {
			double ret = ((IPipeTransportAirHook) this.container.pipe).receiveEnergy(from, val);
			if (ret >= 0) {
				return ret;
			}
		}
	...
	public void requestEnergy(ForgeDirection from, double amount) {
		...
		if (this.container.pipe instanceof IPipeTransportAirHook) {
			nextPowerQuery[from.ordinal()] += ((IPipeTransportAirHook) this.container.pipe).requestEnergy(from, amount);
		} else {
	...
	static {
		powerCapacities.put(PipeAirCobblestone.class, 8);
		powerCapacities.put(PipeAirStone.class, 16);
		powerCapacities.put(PipeAirWood.class, 32);
		powerCapacities.put(PipeAirQuartz.class, 64);
		powerCapacities.put(PipeAirGold.class, 256);
		powerCapacities.put(PipeAirDiamond.class, 1024);
	}
